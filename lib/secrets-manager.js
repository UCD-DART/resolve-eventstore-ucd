...
const getSecret = async (pool, selector) => {
    const log = get_log_1.default('secretsManager:getSecret');
    log.debug(`retrieving secret value from the database`);
    const { databaseName, secretsTableName, escapeId, executeStatement, escape, } = pool;
    // TODO: refactor
    if (!secretsTableName ||
        !escapeId ||
        !databaseName ||
...
const setSecret = async (pool, selector, secret) => {
    const log = get_log_1.default('secretsManager:setSecret');
    log.debug(`setting secret value within database`);
    const { databaseName, secretsTableName, escape, escapeId, executeStatement, } = pool;
    // TODO: refactor
    if (!secretsTableName ||
        !escapeId ||
        !databaseName ||
...
const deleteSecret = async (pool, selector) => {
    const log = get_log_1.default('secretsManager:deleteSecret');
    log.debug(`removing secret from the database`);
    const { databaseName, secretsTableName, escapeId, escape, executeStatement, } = pool;
    // TODO: refactor
    if (!secretsTableName ||
        !escapeId ||
        !escape ||
...
const getSecretsManager = (pool) => {
    const log = get_log_1.default('getSecretsManager');
...
exports.default = getSecretsManager;
