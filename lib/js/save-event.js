...
const saveEvent = async ({ databaseName, eventsTableName, executeStatement, escapeId, escape }, event) => {
    while (true) {
        try {
            ...
            const byteLength = Buffer.byteLength(serializedEvent) + constants_1.RESERVED_EVENT_SIZE;
          ...
          GREATEST(
            CAST(extract(epoch from clock_timestamp()) * 1000 AS ${constants_1.LONG_NUMBER_SQL_TYPE}),
            ${+event.timestamp}
          ),
          ${serializedEvent},
          ${byteLength}
            ...
            if (errorMessage.indexOf('subquery used as an expression') > -1) {
                ...
                throw new resolve_eventstore_base_1.ConcurrentError(event.aggregateId);
            }
            else {
                throw error;
            }
