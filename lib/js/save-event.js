...
const saveEvent = async ({
  ...
  while (true) {
    try {
      const serializedEvent = [`${escape(event.aggregateId)},`, `${+event.aggregateVersion},`, `${escape(event.type)},`, escape(JSON.stringify(event.payload != null ? event.payload : null))].join(''); // TODO: Improve calculation byteLength depend on codepage and wide-characters

      const byteLength = Buffer.byteLength(serializedEvent) + _constants.RESERVED_EVENT_SIZE;
      ...
      const eventsTableAsId = escapeId(eventsTableName); // prettier-ignore

      await executeStatement(`WITH "freeze_check" AS (
          ...
          GREATEST(
            CAST(extract(epoch from clock_timestamp()) * 1000 AS ${_constants.LONG_NUMBER_SQL_TYPE}),
            ${+event.timestamp}
          ),
          ${serializedEvent},
          ${byteLength}
      ...
      const errorMessage = error != null && error.message != null ? error.message : '';
      ...
      if (errorMessage.indexOf('subquery used as an expression') > -1) {
        ...
        throw new _resolveEventstoreBase.ConcurrentError(event.aggregateId);
      } else {
        throw error;
      }
    }
