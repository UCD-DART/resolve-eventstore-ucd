...
const commitIncrementalImport = async ({ executeStatement, databaseName, eventsTableName, escapeId, escape }, importId, validateAfterCommit) => {
    ...
    `);
        ...
        FROM ${databaseNameAsId}.${threadsTableAsId}`)).map(({ threadId, threadCounter }) => ({
                threadId: !isNaN(+threadId) ? +threadId : Symbol('BAD_THREAD_ID'),
                threadCounter: !isNaN(+threadCounter)
                    ? +threadCounter
                    : Symbol('BAD_THREAD_COUNTER'),
            ...
            for (const { threadId, threadCounter } of predictedThreadIdCounters) {
                if (validationMapReal.get(threadId) !== threadCounter - 1 &&
            ...
            if (validationErrors.length > 0) {
                const compositeError = new Error(validationErrors.map(({ message }) => message).join('\n'));
...
exports.default = commitIncrementalImport;
