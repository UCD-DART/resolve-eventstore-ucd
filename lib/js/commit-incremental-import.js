...
const commitIncrementalImport = async ({
  ...
  try {
    ...
    if (validateAfterCommit != null && validateAfterCommit === true) {
      const realThreadIdCounters = (await executeStatement(`SELECT "threadId", MAX("threadCounter") AS "threadCounter"
        FROM ${databaseNameAsId}.${eventsTableAsId}
        GROUP BY "threadId"
        `)).map(({
        threadId,
        ...
        threadCounter: !isNaN(+threadCounter) ? +threadCounter : Symbol('BAD_THREAD_COUNTER')
      }));
      const predictedThreadIdCounters = (await executeStatement(`SELECT "threadId", "threadCounter"
        FROM ${databaseNameAsId}.${threadsTableAsId}`)).map(({
        threadId,
        threadCounter
      }) => ({
        threadId: !isNaN(+threadId) ? +threadId : Symbol('BAD_THREAD_ID'),
      ...
      for (const {
        ...
        if (validationMapPredicted.get(threadId) !== threadCounter + 1) {
          validationErrors.push(new Error(`Real -> Predicted threadCounter mismatch ${threadId} ${threadCounter} ${validationMapPredicted.get(threadId)}`));
        }
      }
      ...
      for (const {
        ...
        if (validationMapReal.get(threadId) !== threadCounter - 1 && validationMapReal.get(threadId) != null) {
          validationErrors.push(new Error(`Predicted -> Real threadCounter mismatch ${threadId} ${threadCounter} ${validationMapReal.get(threadId)}`));
        }
      }

      if (validationErrors.length > 0) {
        const compositeError = new Error(validationErrors.map(({
          message
        }) => message).join('\n'));
        compositeError.stack = validationErrors.map(({
          stack
        }) => stack).join('\n');
    ...
    if (error != null && (/Table.*? does not exist$/i.test(error.message) || /subquery used as an expression/i.test(error.message))) {
...
var _default = commitIncrementalImport;
exports.default = _default;
//# sourceMappingURL=commit-incremental-import.js.map