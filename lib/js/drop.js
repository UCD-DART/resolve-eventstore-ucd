...
const drop = async ({ databaseName, eventsTableName, snapshotsTableName, executeStatement, escapeId, }) => {
    ...
    for (const statement of statements) {
        try {
            ...
            if (error != null && `${error.code}` === '42P01') {
                throw new resolve_eventstore_base_1.EventstoreResourceNotExistError(`Double-free eventstore-postgresql adapter via "${databaseName}" failed`);
            }
            else {
                errors.push(error);
            }
