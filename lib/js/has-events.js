...
const hasEvents = async (pool, events) => {
    ...
    WHERE ${events
        .map(({ aggregateId, aggregateVersion }) => `( "aggregateId" = ${escape(aggregateId)} AND "aggregateVersion" = ${escape(aggregateVersion)} )`)
        .join(' OR ')}`);
    const resultSet = new Set();
    for (const { aggregateId, aggregateVersion } of rows) {
        resultSet.add(`${aggregateId}-${aggregateVersion}`);
    }
    const result = events.map(({ aggregateId, aggregateVersion }) => resultSet.has(`${aggregateId}-${aggregateVersion}`));
    return result;
};
exports.default = hasEvents;
